...
const InfoWindow = ({ info, hoverdata }) => {
  ...
  return (
    ...
    >
      <h1 className="text-[20px] text-[#eaeaea] font-medium w-full max-w-[60rem] tracking-normal overflow-hidden text-ellipsis line-clamp-1 font-['Poppins'] max-[794px]:text-3xl">{info?.title?.english}</h1>
      ...
      <div className="text-[#ddddddf0] text-[14px] font-['poppins'] mt-2 overflow-hidden text-ellipsis line-clamp-3">{info?.description?.replace(/<[^>]*>/g, '')}</div>
      ...
      <Link href={`/watch/${info?.id}`} className="text-white bg-[#0f152565] w-full h-10 rounded-lg mt-4 border-2 border-[#ffffff30] flex items-center justify-center gap-2"><FaCirclePlay /> Watch Now</Link>
    </motion.div>
  )
}
