  ...
  return (
    <div className="bg-[#201f28] w-full max-w-[22rem] EPSResponsive rounded-md flex flex-col">
      ...
      <div className="px-2 overflow-y-scroll h-full max-h-[44rem]">
        {data.map((item, index) => <EpisodeCard key={index} info={item} currentEp={episode} index={index} />)}
        {data === undefined || !data ? <p className="text-[#d5d5d7] text-center mt-5">No episodes found</p> : null}
      </div>
    </div>
  )
}
